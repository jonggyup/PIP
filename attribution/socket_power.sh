...
cal_all_power() {
	...
	power_cpu_zone0=$(echo "$power_cpu_zone0 - 36" | bc)
	power_cpu_zone1=$(echo "$power_cpu_zone1 - 69" | bc)
#	power_cpu_zone0=$(echo "$power_cpu_zone0 - $power_mem_zone0" | bc)
#	power_cpu_zone1=$(echo "$power_cpu_zone1 - $power_mem_zone1" | bc)
	total=$(echo "$power_cpu_zone0 + $power_cpu_zone1" | bc)



        # Print CPU power for both zones on the same line
        echo "$power_cpu_zone0 $power_cpu_zone1 $total"
    done
}
