...
This program dynamically adjusts CPU bandwidth limits for the "user" cgroup
...
whose prediction is at or below the target.
"""
...
def update_feature_map_generic(feature_map, future_throttle, direct_size, available_cores, group):
    ...
    """
    global adjustment_ratio, current_bandwidth, num_cores
...
# -----------------------------
def recover_bandwidth(feature_map):
    """
    For the user group, iterate from 100 down to current throttle +1.
    ...
    for candidate in range(100, current_val, -5):
        ...
        new_map = update_feature_map_generic(new_map, candidate, direct_metrics_size, core_count, "user")
...
#            print(f"Debug: recover | target {target_power} | prediction {prediction} | recover {candidate} | calibration {calibration_offset}", flush=True)
            used_map = True
            return candidate
    return current_val

def predict_featuremap(feature_map, candidate):
    global used_map
    used_map = True
    ...
    modified_map = update_feature_map_generic(modified_map, candidate, direct_metrics_size, core_count, "user")
...
def increase_bandwidth(feature_map, gap):
    """
    Increase bandwidth when actual power is below target.
    
    For user, if the gap is small (<10W), a baby step (+1%) is taken;
    otherwise, the ML prediction (via recover_bandwidth) is used.
    If the ML recovery returns the current throttle (likely due to calibration error),
    then a fallback increment by 5 is forced.
    
    Returns updated current_bandwidth.
    """
    global current_bandwidth, used_map
    used_map = False
...
#            print("Debug: high gap ML recovery for user")
            candidate = recover_bandwidth(feature_map)
...
#                print(f"Debug: fallback increment on user to {new_val} | candidate {candidate}")
        ...
        predict_featuremap(feature_map, new_val)
...
def strict_power_capping(feature_map):
    ...
    For small violations, reduce the user throttle by 5 or 10.
    """
    global current_bandwidth, prev_prediction, retry, predictive_feature_map, prev, calibration_offset, smoothing_cal, actual_power
...
#            print(f"Debug: strict (baby step) | target {target_power} | throttle at minimum", flush=True)
            return current_bandwidth
    if prev == 1:
...
def run_stress_test():
    ...
    """
    global actual_power, current_bandwidth, prev, retry
    ...
    while True:
        ...
        if not full_mode:
            ...
            try:
                ...
                # keep the same print format (bandwidth for user only)
                print(f"{timestamp} | {current_bandwidth} | 100.0 | {target_power} | {actual_power:.0f}", flush=True)
            except Exception as e:
        ...
        # Adjust throttling based on measured power
        if actual_power >= target_power * (1 - ceiling):
            current_bandwidth = strict_power_capping(metrics_data)
            prev = 1
            retry += 1
            used_map = True
        elif actual_power < target_power * (1 - ceiling * 2):
            gap = target_power - actual_power
            current_bandwidth = increase_bandwidth(metrics_data, gap)
            prev = 0
        else:
            retry = 0
            prev = 0
