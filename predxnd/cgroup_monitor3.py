...
def run_perf_for_cores(events, core_list):
    """
    Runs perf stat on a specific list of CPU cores.
    """
    if not core_list:
        return {}
    ...
    # Use -C to specify cores instead of -G for cgroup
    cmd = [
        "perf", "stat", "-a", "-e", events_str,
        "-C", core_str, "-x,", "sleep", "1"
    ]
    try:
        proc = subprocess.run(
            cmd,
...
def main():
    ...
    if not os.path.isdir(cgroup_path):
        # Handle unified hierarchy path for cgroup v2
        cgroup_path = os.path.join(CGROUP_BASE_PATH, "system.slice", cgroup_name)
        if not os.path.isdir(cgroup_path):
    ...
    while True:
        ...
        if not cpuset_cores:
            print(json.dumps({"error": "cgroup has no cores assigned in cpuset.cpus"}), separators=(",", ":"))
            sys.stdout.flush()
            time.sleep(1)
            continue
        ...
        # MODIFIED: Call the new function with the list of cores
        perf_counts = run_perf_for_cores(supported, cpuset_cores)
        ...
        sys.stdout.flush()
        # The perf command already sleeps for 1s, so no extra sleep is needed.
        # time.sleep(1)
