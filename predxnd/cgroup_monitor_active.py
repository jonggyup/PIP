...
def parse_cpu_data(output, cpu_metrics, temps_logical, occupied_cores):
    """
    Parse `cpupower monitor` output; for each occupied core, collect exactly
    ...
    # If header wasn’t found, just return perf + temps
    if c0_index is None:
        parsed.extend(cpu_metrics)
        for c in sorted(occupied_cores):
            parsed.append(temps_logical.get(c, 0.0))
        return parsed, sorted(occupied_cores)

    # 2) Build a map: cpu_id → [C0, Cx, Freq, POLL, C1, C1E, C6]
    want_fields = ["C0", "Cx", "Freq", "POLL", "C1", "C1E", "C6"]
    occupied_set = set(occupied_cores)
    per_core_map = {c: [0.0] * 7 for c in occupied_cores}
    ...
    for line in lines:
        ...
        try:
            ...
            continue
        if cpu_val not in occupied_set:
            continue
    ...
    sorted_cores = sorted(occupied_cores)
...
def main():
    ...
    while True:
        ...
        parsed, sorted_cpus = parse_cpu_data(cp_out, cpu_metrics, temps_logical, occupied)
