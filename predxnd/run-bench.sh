...
# --- Output Directory ---
OUTPUT_DIR="$SCRIPT_DIR/results-evaluation"
mkdir -p "$OUTPUT_DIR"
...
cleanup_target() {
    ...
    sudo pkill -ef merge_pred.py > /dev/null 2>&1
...
for bg_target_info in "${TARGET_APPS[@]}"; do
    ...
    for bg_source_info in "${TARGET_APPS[@]}"; do
        ...
        for target_app_info in "${TARGET_APPS[@]}"; do
            ...
            RESULT_FILE="$OUTPUT_DIR/app-${app_name}_tgt-bg-${bg_target_name}_src-bg-${bg_source_name}.log"
            {
            ...
            nohup python3 -u "./merge_pred.py" user < /dev/null >> "$RESULT_FILE" 2>&1 &
            PRED_PID=$!
            ...
            ssh "$SOURCE_HOST" "
                ...
                while true; do
                    echo '[REMOTE] Starting/Restarting send_print.py...';
                    cd '$SCRIPT_DIR';
                    python3 -u './send_print.py' user $TARGET_HOST_IP;
                    echo '[REMOTE] send_print.py exited. Restarting in 1 second...';
                    sleep 1;
                done                
            " &
